...
| GitLab CLI (glab) | For GitLab | Used for GitLab provider operations; authenticate with `glab auth login` (or set `GITLAB_TOKEN`) |
...
# Select the hosting provider explicitly (also available on plan/apply)
...
- **pre-commit**: Runs a quick RepoLens check (`files`, `git` categories — sensitive files and `.gitignore` hygiene) before each commit. If blocking findings are detected, the commit is aborted.
...
- **files**: `.gitignore` presence and recommended entries (FILE002-003)
- **docs**: README, LICENSE, and documentation quality checks (DOC001-010)
- **security**: Repository security settings — branch protection, vulnerability
  alerts, secret scanning, Actions permissions (SEC007-017)
...
| SEC011 | Critical | Vulnerability alerts disabled |
| SEC012 | Warning | Dependabot security updates disabled (GitHub-only) |
| SEC013 | Warning | Secret scanning disabled |
| SEC014 | Warning | Secret scanning push protection disabled |
| SEC015 | Warning | Actions permissions allow all actions |
| SEC016 | Warning | Default workflow permissions set to write |
| SEC017 | Warning | Fork pull requests do not require approval |
